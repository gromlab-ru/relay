...
`relay-cli init` создаёт проектный конфиг, ID проекта из 8 символов, системные
[доски](BOARDS.md) и [единое хранилище](STORAGE.md). Совместимое представление настроек
через API/config get выглядит так:
...
`storageDir` считается от файла конфигурации: `tasks` означает `.relay/tasks`, а не каталог
в корне репозитория. В едином формате это прежняя привязка для совместимости; текущие
сущности находятся в `entities` рядом с конфигом, блокировка — в runtime этой базы.
...
При наличии `storage.json` эти настройки принадлежат `entities/projects/{id}.json`.
Поле `projectSettings` в приведённом выше ответе — совместимое представление, а не второй
источник на диске. Обычный config.json единого формата содержит только настройки открытия
и окружения. Следующее описание вложенного документа применяется к прежним конфигурациям.

`projectSettings` — необязательный вложенный документ версии 1. `name` содержит
однострочное имя (1–120 символов), `slug` — адрес Web (2–64 строчные латинские буквы,
цифры и одиночные дефисы между частями), `revision` защищает от одновременных правок.
При init имя берётся из каталога, slug создаётся как `project-` и четыре случайных
символа из `a–z` и `0–9`. Значения меняются через страницу настроек проекта.

У прежних конфигураций без этого поля используется имя каталога и стабильный
производный slug с ревизией 0, без записи при чтении. Первая изменяющая запись
добавляет документ; ID и остальные настройки сохраняются. Внешняя версия конфига
остаётся 1. Имя не является названием паспорта продукта. Slug не меняет ID,
каталоги хранения и ключи регистрации workspace. Занятость проверяет сервер среди
подключённых проектов; ссылки по ID остаются действительными.
