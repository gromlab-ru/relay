...
Рабочий режим 0.3.0: сервер запускается с конфигом оркестратора, агентам в отдельных
Git worktree на том же хосте передаются URL и собственный `TASKS_ACTOR`. CLI выбирает
HTTP по `--server-url` / `TASKS_SERVER_URL` / `server.url`. Логи, назначения и прогресс
...
Примеры и ограничения: [справочник CLI](../apps/cli/docs/CLI.md#http-режим-и-local).
