...
- [Движок основных сущностей](ENTITIES.md) — виды, записи, ключи/ID и действия `/entities`.
...
Панель «Проект Relay» в Swagger показывает доступные проекты workspace. Выберите проект
перед Execute: локальный маршрут автоматически получает `/projects/<постоянный-ID>/`,
а готовый curl показывает итоговый URL. Выбор принадлежит вкладке, не глобальному состоянию
сервера. Явно проектный маршрут использует свой параметр project; серверные операции
доступны без выбора. Каноническая OpenAPI сохраняет оба варианта для SDK.
