...
`graph context <ref>` использует те же параметры и профиль all по умолчанию.
Значение context — совместимое имя полного обхода. Результат содержит только явно
сохранённые связи Core, без рёбер из продуктовых полей и скрытого отсечения видов сущностей.
