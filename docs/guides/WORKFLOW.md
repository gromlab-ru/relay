...
```bash
npx @gromlab/relay-cli create "Регистрация пользователей" --group product --assignee orchestrator --actor orchestrator
npx @gromlab/relay-cli create "API регистрации" --parent 1 --group backend --actor orchestrator --stdin <<'MD'
## Требования
...
MD
npx @gromlab/relay-cli create "Форма регистрации" --parent 1 --depends-on 2 \
  --group frontend --actor orchestrator --description "Отправить данные в API и показать результат"
npx @gromlab/relay-cli status 1 in_progress --actor orchestrator
npx @gromlab/relay-cli tree 1
...
```bash
npx @gromlab/relay-cli list --ready --group backend
npx @gromlab/relay-cli update 2 --assignee backend-agent --status in_progress --actor orchestrator
```

Субагент получает ID `2`, читает контекст и записывает ход работы:

```bash
npx @gromlab/relay-cli get 2
npx @gromlab/relay-cli log add 2 --kind decision --title "Контракт ошибок" \
  --text "Для занятого email возвращаем code EMAIL_TAKEN" --actor backend-agent
npx @gromlab/relay-cli get 2 --fields id,status,assignee,revision --format json
```
...
```bash
npx @gromlab/relay-cli update 2 --if-revision 3 --actor backend-agent \
  --summary "Обработчик готов, выполняются проверки ответов 201 и 409"
npx @gromlab/relay-cli summary 2
npx @gromlab/relay-cli status 2 review --actor orchestrator
```
...
```bash
npx @gromlab/relay-cli list --status review
npx @gromlab/relay-cli comment add 2 --actor orchestrator \
  --text "В ответе 409 отсутствует code. Добавьте EMAIL_TAKEN и проверку поля."
npx @gromlab/relay-cli status 2 in_progress --actor orchestrator
npx @gromlab/relay-cli comment list 2
```

Исполнителем остаётся backend-агент. После исправления он записывает результат:

```bash
npx @gromlab/relay-cli update 2 --actor backend-agent \
  --summary "POST /users готов: 201 при регистрации, 409 с EMAIL_TAKEN при повторном email"
npx @gromlab/relay-cli log add 2 --kind summary --title "API готов к проверке" \
...
MD
npx @gromlab/relay-cli status 2 review --actor orchestrator
npx @gromlab/relay-cli get 2 --full
```

После фактической проверки оркестратор принимает API:

```bash
npx @gromlab/relay-cli comment add 2 --text "Контракт и проверки приняты" --actor orchestrator
npx @gromlab/relay-cli status 2 done --actor orchestrator
```
...
```bash
npx @gromlab/relay-cli list --ready --group frontend
npx @gromlab/relay-cli update 3 --assignee frontend-agent --status in_progress --actor orchestrator
```

Frontend-субагент получает ID `3` и читает контекст:

```bash
npx @gromlab/relay-cli get 3
npx @gromlab/relay-cli summary 2
npx @gromlab/relay-cli log search 2 --query "EMAIL_TAKEN" --kind summary
```

После реализации формы и проверки сценариев:

```bash
npx @gromlab/relay-cli update 3 --actor frontend-agent \
  --summary "Форма подключена: показывает успех при 201 и сообщение при EMAIL_TAKEN"
npx @gromlab/relay-cli log add 3 --kind summary --actor frontend-agent \
  --text "Проверены регистрация нового пользователя и ошибка повторного email"
npx @gromlab/relay-cli status 3 review --actor orchestrator
npx @gromlab/relay-cli status 3 done --actor orchestrator
npx @gromlab/relay-cli tree 1
```
...
```bash
npx @gromlab/relay-cli update 1 --summary "API и форма приняты, регистрация готова" --actor orchestrator
npx @gromlab/relay-cli log add 1 --kind summary --actor orchestrator \
  --text "Задачи 2 и 3 завершены, проверен сквозной сценарий регистрации"
npx @gromlab/relay-cli status 1 done --actor orchestrator
npx @gromlab/relay-cli list --all
npx @gromlab/relay-cli validate
```
...
```bash
npx @gromlab/relay-cli comment add 3 --text "На узком экране кнопка перекрывает ошибку" --actor human
npx @gromlab/relay-cli status 1 in_progress --actor orchestrator
npx @gromlab/relay-cli status 3 in_progress --actor orchestrator
npx @gromlab/relay-cli update 3 --actor frontend-agent \
...
```bash
npx @gromlab/relay-cli assign 3 frontend-agent-2 --actor orchestrator
```
...
```bash
npx @gromlab/relay-cli get 3
npx @gromlab/relay-cli comment list 3
npx @gromlab/relay-cli log list 3 --limit 5
```
...
```bash
npx @gromlab/relay-cli create "Регистрация по SMS" --group product --actor orchestrator
npx @gromlab/relay-cli comment add 4 --text "Исключено из первой версии" --actor orchestrator
npx @gromlab/relay-cli status 4 cancelled --actor orchestrator
npx @gromlab/relay-cli list --status cancelled
npx @gromlab/relay-cli validate
```
