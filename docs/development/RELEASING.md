...
Тег `mcp-v0.1.0` относится к прежнему пакету Tasks MCP. Первая автоматическая
публикация Relay MCP использует `mcp-v0.1.1`; первоначальная версия `0.1.0`
опубликована локально.
...
Шаг публикации запускает соответствующий `pnpm run release:publish…`: npm-обёртке
нужен `npm_execpath`, который задаёт pnpm. Прямой запуск через `node` подходит
для проверки метаданных и описания релиза, но не для новой npm-публикации.
