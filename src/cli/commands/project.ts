...
import { migrateTasks } from "../../application/tasks/migrate.js";
...
export function registerProject(program: Command, runtime: Runtime): void {
  ...
  registerCommand(program, runtime, {
    ...
    run: (context) => validateWorkspace(context.workspace),
  });
  registerCommand(program, runtime, {
    name: "migrate",
    description: "Перевести старые UUID-задачи на числовые ID",
    details:
      "Переход формата v1 → v2: сохраняет существующие уникальные номера как ID,\nназначает остальные выше максимума, переносит ссылки, комментарии и отчёты.\nИсходные файлы и таблица соответствия сохраняются в .runtime/migration-… .\nПрерванный запуск продолжается повторной командой. Для нового формата это пустая операция.",
    examples: [
