...
Разрабатываемая версия включает каркас NestJS и React в том же npm-пакете:
...
В текущем каркасе React проверяет связь с NestJS через health/context.
Полноценный редактируемый канбан, задачные REST-маршруты и SSE описаны как
следующие этапы в [PLAN.md](PLAN.md); задание фронтенд-агенту — [UI_SPEC.md](UI_SPEC.md).
Опубликованная версия может отставать от локально собранного каркаса.
