...
import type { LegacyTask } from "../src/domain/legacy.js";
...
async function seed(root: string, tasks: LegacyTask[]) {
  for (const task of tasks)
    await writeFile(join(root, ".tasks", "tasks", `${task.id}.json`), JSON.stringify(task));
}
...
test("миграция сохраняет номера, разрешает дубликаты и переносит весь контекст со ссылками", async (t) => {
  ...
  const files = (await readdir(join(app.root, ".tasks", "tasks"))).sort();
  assert.deepEqual(files, ["4.json", "5.json", "6.json"]);
  const before = await Promise.all(
    files.map((file) => readFile(join(app.root, ".tasks", "tasks", file), "utf8")),
  );
...
test("ошибка старых связей обнаруживается до записи и оставляет исходные файлы целыми", async (t) => {
  ...
  const path = join(app.root, ".tasks", "tasks", `${task.id}.json`);
  const before = await readFile(path, "utf8");
  failed(await app.run(["migrate"]), "MISSING_REFERENCE", 4);
  assert.equal(await readFile(path, "utf8"), before);
  assert.deepEqual(await readdir(join(app.root, ".tasks", "tasks")), [`${task.id}.json`]);
});
...
test("миграция восстанавливается после остановки на каждой границе публикации", async (t) => {
  for (const point of ["journal", "backup", "publish"]) {
    await t.test(point, async (t) => {
      ...
      const moduleUrl = new URL("../dist/application/tasks/migrate.js", import.meta.url).href;
      const workspaceUrl = new URL("../dist/storage/workspace.js", import.meta.url).href;
      const child = spawn(
        process.execPath,
        [
          "--input-type=module",
        ...
        fs.rename = async (from, to) => {
          ...
          if ((process.env.POINT === 'backup' && String(to).endsWith('/original')) ||
              (process.env.POINT === 'publish' && String(to) === process.env.ROOT + '/.tasks/tasks'))
