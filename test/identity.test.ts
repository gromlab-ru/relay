...
import { getTask } from "../src/application/tasks/queries.js";
...
test("единственный ID — число от 1 во всех ответах, файлах и ссылках", async (t) => {
  ...
  assert.deepEqual((await readdir(join(app.root, ".tasks", "tasks"))).sort(), ["1.json", "2.json"]);
...
test("пропуски ID не заполняются: новая задача получает max + 1", async (t) => {
  ...
  await rm(join(app.root, ".tasks", "tasks", "2.json"));
  assert.equal(await app.create("После пропуска"), 4);
  const path = join(app.root, ".tasks", "tasks", "4.json");
  const task = JSON.parse(await readFile(path, "utf8")) as Task;
  task.id = Number.MAX_SAFE_INTEGER;
  await writeFile(join(app.root, ".tasks", "tasks", `${task.id}.json`), JSON.stringify(task));
  await rm(path);
