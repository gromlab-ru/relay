...
MCP всегда вызывает этот SDK. При отсутствии URL проекта `apps/mcp` автоматически
поднимает локальный REST runtime, затем передаёт его адрес адаптеру. Сгенерированные
операции и контракты остаются едиными для web, CLI и MCP.
