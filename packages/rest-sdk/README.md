...
Для генерации OpenAPI сервер запускается из сборки TypeScript с метаданными декораторов.
В запуске через tsx эти метаданные могут отсутствовать: параметры пути становятся `any`
в SDK. После генерации проверяйте diff: при правке описания должны меняться только описания,
а не типы параметров и сигнатуры операций.
