...
export const boardTaskReferenceSchema = z
  .string()
  .regex(/^(?:[A-Za-z0-9]{8}|[A-Z][A-Z0-9]{1,15}-[1-9]\d*)$/)
  .describe("ID задачи или ключ, например WEB-24");
...
export const boardTaskSavedSchema = z.strictObject({
  ...
  revision,
  action: z.enum(["create", "update", "move", "link"]).describe("Выполненное действие"),
...
export const createBoardTaskSchema = z.strictObject({
  ...
  parentId: boardTaskIdSchema
    .optional()
    .describe("ID родителя: создание подзадачи одной атомарной операцией"),
