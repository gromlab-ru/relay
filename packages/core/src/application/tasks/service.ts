...
import { appendRank } from "../../domain/rank.js";
...
export class TaskService {
  ...
  async create(input: Partial<TaskFields> & { title: string }, actor: string): Promise<Task> {
    ...
    return this.workspace.locked(async (assertOwned) => {
      ...
      const status = fields.status ?? this.workspace.config.defaultStatus;
      const task = parse(
        taskSchema,
        {
          ...initialTaskFields(),
          ...fields,
          status,
          rank: fields.rank ?? appendRank(status, tasks.values()),
          version: 2,
  ...
  async mutate(
    ...
    return this.workspace.locked(async (assertOwned) => {
      ...
      if (patch.status !== undefined && patch.status !== current.status && patch.rank === undefined)
        patch.rank = appendRank(patch.status, tasks.values());
