...
async function publish(workspace: Workspace, state: MigrationState, assertOwned: () => void) {
  ...
  if (!(await exists(backup))) {
    ...
    await syncDirectory(dirname(workspace.root));
  ...
  return {
    ...
    total: state.total,
    flattened: state.nested,
    backupPath: state.nested ? join(backup, "tasks") : backup,
    storageBackupPath: backup,
...
export async function migrateTasks(workspace: Workspace, actor: string) {
  ...
  return workspace.locked(async (assertOwned) => {
    ...
    // Завершаем журнал прежней версии перед переносом всей старой структуры.
    const oldRuntime = workspace.path(".runtime");
    const oldMarker = join(oldRuntime, MIGRATION_STATE);
    if (await exists(oldMarker)) {
      const oldState = parse(
        z.strictObject({
          version: z.literal(1),
          directory: z.string().regex(/^migration-[a-f0-9]{32}$/),
        ...
        await readJson(oldMarker),
        oldMarker,
        true,
      );
      const directory = join(oldRuntime, oldState.directory);
      const target = workspace.path("tasks");
      if (!(await exists(join(directory, "original")))) {
        assertOwned();
        await rename(target, join(directory, "original"));
      }
      if (await exists(join(directory, "tasks"))) {
        invariant(
          !(await exists(target)),
          "MIGRATION_CONFLICT",
          "Каталог задач появился во время миграции",
        ...
        assertOwned();
        await rename(join(directory, "tasks"), target);
      }
      invariant(
        await exists(target),
        "INVALID_DATA",
        "Не найден подготовленный каталог миграции",
        5,
      );
      await syncDirectory(workspace.root);
      await syncDirectory(directory);
      await rm(oldMarker);
      await syncDirectory(oldRuntime);
    }

    const nested = await exists(workspace.path("tasks"));
    const source = nested ? workspace.path("tasks") : workspace.root;
    invariant(
      !nested || (await jsonFiles(workspace.root)).length === 0,
      "MIGRATION_CONFLICT",
      "JSON-задачи найдены и в корне, и в tasks/. Объедините хранилище перед миграцией.",
      4,
    );

    const current: Task[] = [];
    const legacy: LegacyTask[] = [];
    for (const filename of await jsonFiles(source)) {
    ...
    if (
      !legacy.length &&
      !nested &&
      !(await exists(oldRuntime)) &&
      !(await exists(workspace.path(".gitignore")))
    )
      return { migrated: 0, total: current.length, flattened: false };
