...
export async function validateWorkspace(workspace: Workspace) {
  return workspace.locked(async () => {
    ...
    for (const file of await jsonFiles(workspace.root)) {
      try {
        const task = await repository.readFile(file);
        tasks.set(task.id, task);
      } catch (error) {
        const failure = asAppError(error);
        issues.push({ path: file, code: failure.code, message: failure.message });
      }
    }
    ...
    return { valid: true, tasks: tasks.size, comments, logs };
  });
}
