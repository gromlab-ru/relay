...
Подключены к постоянным данным разделы [планов работ](../../docs/product/applications/web/PLANS.md)
...
| domains/planning                           | Постоянные планы, этапы, выбор задач, операции и SWR          |
| compositions/screens/releases              | Самостоятельный каталог, просмотр и форма релизов             |
| domains/releases                           | Статус, состав, готовность и чтение серверного снимка выпуска |
| compositions/widgets/entity-history        | Общее постраничное чтение истории плана и релиза              |
...
Планы и релизы читаются с сервера; каталоги, этапы, задачи и состав имеют отдельные
страницы. Прогресс вычисляется в Core по всему составу. Черновики `server-v1` сохраняют
исходную ревизию по проекту и вкладке; неподтверждённая отправка сохраняет requestId.
SSE не затирает ввод. Статусы планов, задач и релизов независимы. Снимок выпуска содержит
полные тексты и не пересчитывается после изменений источников. Материалы и история
доступны у владельцев. Старые примеры браузера автоматически не импортируются.
[Досье и фактические проверки](../../docs/work/planning/README.md).
