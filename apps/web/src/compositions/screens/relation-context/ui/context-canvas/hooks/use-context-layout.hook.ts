...
export const useContextLayout = (
  nodes: RelationNode[], edges: RelationEdge[], root: string, selectedNodeId: string | null,
) => {
  const [layout, setLayout] = useState<ContextLayout>(() => ({
    nodes: new Map(), edges: new Map(), distances: new Map(), parents: new Map(),
  }));
  ...
  useEffect(() => { anchorRef.current = selectedNodeId ?? root; }, [selectedNodeId, root]);
  useEffect(() => {
    let isActive = true;
    const elk = new ELK({ workerFactory: () => new Worker(elkWorkerUrl) });
    const plan = createContextLayoutPlan(nodes, edges, root);
    setPending(true);
    setError(null);
    void elk.layout(plan.graph).then((result) => {
      if (!isActive) return;
      const anchorId = anchorRef.current;
      const previousAnchor = layoutRef.current.nodes.get(anchorId);
      const next = readContextLayout(result, plan,
        previousAnchor ? { id: anchorId, position: previousAnchor.position } : undefined);
      layoutRef.current = next;
      setLayout(next);
    }).catch(() => {
      if (isActive) setError("Не удалось рассчитать расположение. Повторите раскладку или откройте список.");
    }).finally(() => {
      elk.terminateWorker();
      if (isActive) setPending(false);
    });
    return () => { isActive = false; elk.terminateWorker(); };
  }, [nodes, edges, root, attempt]);
  ...
  return { layout, isPending, error, onMove: handleMove, onRetry: () => setAttempt((value) => value + 1) };
};
