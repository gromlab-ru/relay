...
```bash
pnpm --filter @relay/web run create ui-component context-node-relations src/compositions/screens/relation-context/ui/context-inspector/ui
```

Внутренний компонент инспектора без фасета показывает все загруженные отношения
выбранной сущности по восемь с продолжением, в том числе дополнительные связи дерева.
Начальные типы, стили и разметка адаптированы к фактическому контракту.
