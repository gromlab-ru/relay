...
Начиная с **0.3.0**, CLI поддерживает локальный Core и HTTP через общий SDK.
Сервер должен поддерживать HTTP-режим версии 0.3.0. При подключении к старому
серверу CLI сообщает `SERVER_INCOMPATIBLE`.

Для постоянного адреса добавьте URL в существующий `tasks.config.json`:
...
**Оркестратор** запускает сервер с конфигом своей рабочей копии:

```bash
npx @gromlab/tasks-cli server --config /work/orchestrator/tasks.config.json --actor orchestrator
```

**Каждый агент** работает в собственном Git worktree на том же хосте. URL можно
получить из скопированного конфига или передать через окружение, в том числе при
динамическом порте сервера. При URL из окружения локальный конфиг необязателен:

```bash
export TASKS_SERVER_URL=http://127.0.0.1:3000
export TASKS_ACTOR=agent-1

npx @gromlab/tasks-cli list --ready
npx @gromlab/tasks-cli claim 12 --status in_progress
npx @gromlab/tasks-cli log add 12 --kind progress --session-id wave-1 \
  --request-id agent-1-step-1 --text "Готов обработчик. Следующий шаг — проверки."
npx @gromlab/tasks-cli update 12 --summary "Обработчик готов, выполняются проверки"
npx @gromlab/tasks-cli status 12 review
```

Все чтения и изменения задач направляются серверу. Записи сохраняются в `.tasks`,
указанной **серверным конфигом**, и доступны оркестратору после завершения агента.
Сервер пишет в свою рабочую копию; Git-коммиты и слияние кода выполняет оркестратор.
`summary` удобно использовать для актуального состояния, `log` — для последовательности
решений, результатов и следующих шагов. Для замены текста после чтения доступна `--if-revision`.

Приоритет: **`--local` → `--server-url` → `TASKS_SERVER_URL` → `server.url`**.
Без URL используется Core. Если сервер недоступен, CLI завершает команду ошибкой.
Для аварийной записи оркестратор явно выбирает общее хранилище:

```bash
npx @gromlab/tasks-cli --local --config /work/orchestrator/tasks.config.json \
  log add 12 --actor orchestrator --kind progress \
  --request-id recovery-step-1 --text "Сервер остановлен; контекст сохранён локально."
```

`--local` работает и при запущенном сервере: Core использует ту же блокировку,
а наблюдатель SSE сообщает об изменении. Путь `--config` определяет рабочую копию,
в которую выполняется локальная запись.

Повтор добавления с тем же `--request-id`, задачей, типом записи, автором и содержимым
возвращает прежнюю запись без увеличения revision. Другой текст или автор с тем же
ключом дают `IDEMPOTENCY_CONFLICT`. Без флага CLI создаёт ключ автоматически и при
неподтверждённом POST возвращает его в `error.details.requestId`. Повторяйте эту же
запись с тем же ключом. Создание задач и изменение полей автоматически не повторяются.

Подробные правила: [HTTP-режим CLI](docs/CLI.md#http-режим-и-local).
