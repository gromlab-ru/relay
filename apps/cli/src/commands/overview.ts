...
export function registerOverview(program: Command, runtime: Runtime): void {
  registerCommand<OverviewOptions>(program, runtime, {
    ...
    examples: [
      ["relay-cli overview", "Понять состояние проекта и выбрать следующее действие"],
      ["relay-cli overview 1 --limit 20", "Посмотреть крупное направление и его зависимости"],
      ["relay-cli overview 9 --format json", "Получить структурированную сводку выбранного дерева"],
      [
        "relay-cli overview --review-status qa,acceptance",
